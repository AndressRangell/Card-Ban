...
/**
 * <code>SunJSSESocketFactory</code> is used by BaseChannel and ISOServer in
 * order to provide hooks for SSL implementations.
 *
 * @version $Revision: 2706 $ $Date: 2009-03-05 09:24:43 -0200 (Thu, 05 Mar
 * 2009) $
 * @author Bharavi Gade
...
public class AndroidSocketFactory
    ...
    String clase = "AndroidSocketFactory.java";
...
/*    private int timeout;
    ...
    public AndroidSocketFactory(InputStream fileInputStream) {
        super();
    ...
    private SSLContext getSSLContext() throws ISOException {
        ...
        try {
            ...
            SSLContext sslc = SSLContext.getInstance(this.algorithm);

            sslc.init(kma, tma, null);
            return sslc;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Socket createSocket(String host, int port)
       ...
       /* SSLSocket s = (SSLSocket) socketFactory.createSocket();
    ...
    private void verifyHostname(SSLSocket socket)
        ...
        if (serverName == null || serverName.length() == 0) {
            ...
            try {
                ...
                uhe.printStackTrace();
                Logger.logLine(LogType.FLUJO, clase, uhe.getStackTrace());
                Logger.logLine(LogType.EXCEPTION, clase, uhe.getMessage());
                Logger.logLine(LogType.EXCEPTION, clase, uhe.getStackTrace());
    ...
    /**
     * @see org.jpos.core.Configurable#setConfiguration(Configuration)
     */
    ...
    public Socket createSocket(String host, int port, int timeout)
        ...
        socketConn.connect(new InetSocketAddress(host, port), timeout);
        SSLSocket s = (SSLSocket) socketFactory.createSocket(socketConn, host, port, true);
        verifyHostname(s);
        return s;
    }
