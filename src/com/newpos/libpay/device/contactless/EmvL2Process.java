...
import com.cobranzas.file_management.Files_Management;
...
public class EmvL2Process {
    public EmvL2Process(Context context, TransUI p) {
        mContext = context;
        Tparam = p;
    }
    ...
    byte[] PayGenericTag55 = {
            ...
            (byte) 0x5F, (byte) 0x2A, // Transaction Currency Code
            (byte) 0x9F, (byte)0x6E,
            (byte) 0x84 //Nombre de archivo dedicado
    };
    ...
    public int emvl2ParamInit() {
        ...
        modules = new String[]{path + "libPayPass.so", path + "libPayWave.so", path + "libXPressPay.so", path + "libDPAS.so", path + "libJSpeedy.so"};
        ...
        File tempFile = new File(mContext.getFilesDir().getPath() + "/libJSpeedy.so");
        if (!tempFile.exists()) {
            ...
            PAYUtils.copyAssetsToData(mContext, "libJSpeedy.so");
        }
        ...
        //load param
        //byte[] su = PAYUtils.getAssertFileData("TERMINAL",mContext);
        ...
        //byte[] aid = PAYUtils.getAssertFileData("PROCESSING",mContext);
        ...
        //byte[] entrypoint = PAYUtils.getAssertFileData("ENTRY_POINT",mContext);
    ...
    public int start() {
        ...
        Logger.debug("=======Before transactionSetData=======: " + param.otherAmount);

        transactionSetData(param);

        Logger.debug("=======After transactionSetData=======: " + param.otherAmount);
        ...
        if (TranResult == IEmvL2CallBack.L2_APPROVED)
            return 0;
        else if (TranResult == IEmvL2CallBack.L2_DECLINED) {
            return 7;
        } else
            return TranResult;
    }
...
//for print
    ...
    private void transactionSetData(ParamEmvL2 param) {
        ...
        byte[] amount = ISOUtil.str2bcd(String.valueOf(param.amount/100), true);
        System.arraycopy(amount, 0, trans_data, trans_data_len + 6 - amount.length, amount.length);
        trans_data_len += 6;

        Logger.debug("EMVL2 OtherAmount: " + param.otherAmount);

        if (param.otherAmount > 0) {

            Logger.debug("Inside OtherAmount: " + param.otherAmount);
            trans_data[trans_data_len++] = (byte) 0x9F;//transaction other amount
            trans_data[trans_data_len++] = 0x03;
            trans_data[trans_data_len++] = 0x06;
            byte[] otheramount = ISOUtil.str2bcd(String.valueOf(param.otherAmount/100), false);
