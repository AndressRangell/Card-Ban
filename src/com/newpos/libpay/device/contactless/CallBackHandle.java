...
public class CallBackHandle implements IEmvL2CallBack {
    ...
    static String clase = "CallBackHandle.java";
...
public void FnDisplay(EmvL2App handle, byte clearScreen, byte lan, byte msgid) {
    ...
    //       transUI.handling();
    switch (msgid) {
        case 28://Este mensaje [Error use otra tarjeta] no se mostrara para tarjetas UPI
            if (AID != null && !AID.contains("A000000333")) {
                Tparam.showMessage(statuMsg, true);
            }
            break;
        default:
            Tparam.showMessage(statuMsg, true);
...
//     * transaction is processing.
    ...
    public void FnDisplayAdvance(EmvL2App handle, EmvL2UiRequestData UIRequestData, int AsOutcome) {
        ...
        if(idMsdCase11==-1) {
            ...
            if (UIRequestData.mMessage != 30) {
                switch (UIRequestData.mMessage) {
                    case 28://Este mensaje [Error use otra tarjeta] no se mostrara para tarjetas UPI
                        if (AID != null && !AID.contains("A000000333")) {
...
//app selecting.used in iccard.If it's NFC card.return the app NO which is the highest
 //    * priority.defualt parameter is 0.
    ...
    public int FnAppSelect(EmvL2App handle, int AppCount, EmvL2AppItem[] apps, boolean CL) {
        ...
        int temp=-1;
        int priority_int=0;
        if (AppCount == 1)//one application and contactless will select first one. || CL
            return 0;
...
//     * in the parameter,key and return ture.otherwise return false.
    ...
    public boolean FnCAPublicKeyGet(EmvL2App handle, byte[] RID, byte index, EmvL2PubKey[] key) {
        ...
        //byte[] su = PAYUtils.getAssertFileData(CAKEY,mCtx);
...
// check certification revoke according to RID , index and sn.
    ...
    public boolean FnCertRevokeCheck(EmvL2App handle, byte[] RID, byte index, byte[] sn) {
        ...
        //byte[] revoks = PAYUtils.getAssertFileData(REVOK,mCtx);
        if (revoks !=null){
...
// get tag list data.
     ...
     * @return
     */
    private boolean isAID(byte[] data) {
        String hex = "";
        int len = data.length;
        ...
        try {
            if (len > 2 && (hex.contains("4F"))) {

                int offset;

                offset = hex.indexOf("4F");
                offset += 2;

                int l = Integer.parseInt(hex.substring(offset, offset + 2));
                AID = hex.substring(offset + 2, offset + 2 + l * 2);
