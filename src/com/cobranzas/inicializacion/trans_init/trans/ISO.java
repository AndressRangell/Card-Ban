...
public class ISO {
    ...
    public boolean setField(int id, String content) {
        try {
            ...
            if ((fieldDefinition[(int) id].typeLenght == (byte) ISO.typeLenghtFIX) &&
                ...
                tmp = content.getBytes();
                if (tmp.length <= 65000) {
                    pointerOutput = SetLenghtBin(output, pointerOutput, (int) tmp.length);
                    System.arraycopy(tmp, 0, output, pointerOutput, tmp.length);
                    pointerOutput += tmp.length;
                    return true;
                } else
    ...
    public void initIsoDefinition(fieldDefinition_t[] isoField) {
        ...
        //field58
        isoField[(int) field_58_RESERVED_NATIONAL] = new fieldDefinition_t(ISO.typeLenghtFLLLVAR,
                ISO.typeContentATTAN,
                8);
        ...
        isoField[(int) field_61_RESERVED_PRIVATE] = new fieldDefinition_t(ISO.typeLenghtFLLLVAR,
                ...
                8);
        //field62
        isoField[(int) field_62_RESERVED_PRIVATE] = new fieldDefinition_t(ISO.typeLenghtFHHHVAR,
                ISO.typeContentATTAN,
                8);
        //field63
    ...
    public static void incStan() {
        ...
        SharedPreferences.saveValueIntPreference(Tools.getCurrentContext(), SharedPreferences.keyStan, stan);
    }

    public static void decStan() {
        int stan;
        stan = SharedPreferences.getValueIntPreference(Tools.getCurrentContext(), SharedPreferences.keyStan);
        if (stan > 0)
            stan--;
        else stan = 0;
